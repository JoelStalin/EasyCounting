...
exports.capturePageState = capturePageState;
exports.loadCredentials = loadCredentials;
exports.isPostulacionOpen = isPostulacionOpen;
exports.loginCertPortalIfNeeded = loginCertPortalIfNeeded;
exports.ensureOfvAuthenticated = ensureOfvAuthenticated;
exports.openPostulacionPage = openPostulacionPage;
exports.postulacionFormData = postulacionFormData;
exports.classifyPortalResponse = classifyPortalResponse;
...
function getString(metadata, key, fallback = '') {
...
function parseAuthStrategies(ctx) {
    const raw = ctx.job.target?.metadata?.authStrategies;
    const defaultStrategies = ['session_reuse', 'portal_credentials', 'manual_seed'];
    ...
    const normalized = source
        ...
        .filter((item) => item === 'session_reuse' || item === 'portal_credentials' || item === 'manual_seed');
    return normalized.length > 0 ? normalized : defaultStrategies;
}
function manualSeedTimeoutMs(ctx) {
...
async function classifyPageState(page) {
    ...
    if (current.includes('/postulacion/') ||
...
async function classifyPortalAuthResult(page) {
...
function recordAuthStep(ctx, authFlow, step) {
    authFlow.push(step);
...
async function capturePageState(ctx, page, label) {
    const title = await page.title().catch(() => '');
    const html = await page.content().catch(() => '');
    const aria = await page.locator('body').ariaSnapshot().catch(() => null);
    await ctx.writeArtifact(`${label}.html`, html);
    await ctx.writeArtifact(`${label}.json`, JSON.stringify({
        title,
        url: page.url(),
        aria,
        bodyPreview: (await bodyText(page)).slice(0, 6000),
    }, null, 2));
    const screenshotPath = node_path_1.default.join(ctx.outputDir, `${label}.png`);
...
async function isPostulacionOpen(page) {
    return (await classifyPageState(page)) === 'postulacion_open';
}
async function loginCertPortalIfNeeded(ctx, page, credentials, authFlow) {
    const state = await classifyPageState(page);
    if (state !== 'portal_login') {
    ...
    const waitForSignInPost = () => page
        .waitForResponse((response) => response.request().method().toUpperCase() === 'POST' &&
        response.url().toLowerCase().includes('/portalcertificacion/login/signin'), { timeout: 8000 })
        .then((response) => {
        ...
        .catch(() => ({ posted: false }));
    let signInCapture = { posted: false };
...
async function ensureOfvAuthenticated(ctx, page, credentials) {
...
async function attemptSessionReuseFallbacks(ctx, portalPage, authFlow) {
...
async function attemptManualSeed(ctx, portalPage, authFlow) {
    const state = await classifyPageState(portalPage);
    if (state !== 'portal_login') {
        return false;
    }
...
async function openPostulacionPage(ctx, page, credentials) {
    const authFlow = [];
    if (await isPostulacionOpen(page)) {
        recordAuthStep(ctx, authFlow, {
            strategy: 'session_reuse',
    ...
    const clickedMenuEntry = await page.evaluate(() => {
        ...
        const fallback = Array.from(document.querySelectorAll('td.menu')).find((element) => (element.textContent || '').includes('Solicitud para ser Emisor Electrónico'));
    ...
    let lastAuthResult = await classifyPortalAuthResult(portalPage);
    ...
    throw new Error(`Portal de certificacion abierto pero no se detecto formulario de postulacion. URL=${portalPage.url()} portalAuthResult=${lastAuthResult}`);
}
function postulacionFormData(ctx) {
    const metadata = ctx.job.target?.metadata || {};
...
function classifyPortalResponse(bodyPreview) {
